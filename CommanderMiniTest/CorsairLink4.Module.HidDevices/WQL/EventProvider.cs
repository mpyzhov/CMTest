...
using System.Reactive.Management.Instrumentation.Linq;
...
namespace System.Reactive.Management.Instrumentation
{
    ...
    public static class EventProvider
    {
        ...
        {
            return source.Subscribe(item => item.Fire());
        }
        ...
        internal static IObservable<T> Receive<T>(string wqlQuery, string scope, Func<ManagementBaseObject, T> ctor)
        {
            ...
            return Observable.Create<T>(obs =>
            {
                ...
                var watcher = new ManagementEventWatcher(ms, new EventQuery(wqlQuery));
                watcher.Start();
