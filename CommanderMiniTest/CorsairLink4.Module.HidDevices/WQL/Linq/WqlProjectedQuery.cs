...
namespace System.Reactive.Management.Instrumentation.Linq
{
    ...
    public sealed class WqlProjectedQuery<T, R>
    {
        ...
        /// Selector function.
        /// </summary>
        private readonly Expression<Func<T, R>> selectorFunction;
        ...
        {
            var source = " FROM " + wqlSource.ClassName;

            var filter = string.Empty;
            if (filterPredicate != null)
            {
                filter = " WHERE " + VisitFilter(filterPredicate.Body, filterPredicate.Parameters[0]);
            }
            ...
            {
                project = "SELECT " + VisitSelector(selectorFunction.Body, selectorFunction.Parameters[0], out ctor);
            }

            var within = withinClause.Seconds > 0 ? " WITHIN " + withinClause.Seconds : string.Empty;

            var query = project + source + within + filter;

            if (wqlSource.Logger != null)
            {
                wqlSource.Logger.WriteLine(query);
            }
        ...
        private static string VisitFilter(Expression expression, ParameterExpression parameter)
        {
            switch (expression.NodeType)
            {
                ...
                case ExpressionType.TypeIs:
                    return VisitFilter("ISA", expression, parameter);
                default:
        ...
        private static string VisitFilter(string op, Expression expression, ParameterExpression parameter)
        {
            if (expression is TypeBinaryExpression)
            {
                ...
                var left = VisitFilterOperand(tbe.Expression, parameter);

                string right;
                var a = tbe.TypeOperand.GetCustomAttributes(typeof(WmiClassAttribute), false).Cast<WmiClassAttribute>().SingleOrDefault();
                if (a == null)
                {
                    right = tbe.TypeOperand.Name;
                }
                else
                {
                    right = a.ClassName;
                ...
                return string.Format(CultureInfo.InvariantCulture, "{0} {1} '{2}'", left.ToString(), op, right);
            }
            else
            {
                var be = (BinaryExpression)expression;
